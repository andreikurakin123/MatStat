...
def run_simulation(dist_name, rvs_func, sizes=[10, 100, 1000], iters=1000):
    ...
    for n in sizes:
        # Матрица для хранения результатов 1000 итераций для 5 характеристик
        ...
        # Считаем среднее (E) и корень из дисперсии (стандартное отклонение, sqrt(D))
...
# Настраиваем генераторы для 5 распределений
distributions = {
...
# Запускаем симуляцию (это может занять пару секунд)
